...
const MINIMUN_HEIGHT_WITH_WINDOW = 'A parede deve ser no mínimo 30 centímetros mais alta que a janela';
...
const validateHeight = (height, doors, windows) => {
  if (height < 1) return `Altura ${MIN_VALUE_ERROR}`;
  if (height > 15) return `Altura ${MAX_VALUE_ERROR}`;
  if (doors > 0 && height < 2.2) return MINIMUN_HEIGHT_WITH_DOOR;
  if (windows > 0 && height < 1.5) return MINIMUN_HEIGHT_WITH_WINDOW;
  return null;
};
...
export default ({
  ...
  || validateHeight(height, doors, windows)
